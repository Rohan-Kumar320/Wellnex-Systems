 ...
 * - onExitStart: () => void  (called when preloader begins exit; pages can start reveal)
 * - onExited: () => void     (called when preloader finished exit animation)
 * - brand: string
 * - minDisplayMs: number
 */
      ...
      <div ref={panelRef} className="relative w-full max-w-[920px] p-6 flex flex-col items-center justify-center gap-6" style={{ WebkitTapHighlightColor: "transparent", transform: "translateZ(0)" }}>
        <div className="flex flex-col items-center gap-5">
          <div ref={(el) => setCharRef(el, -1)} className="text-3xl md:text-5xl font-extrabold tracking-widest mt-2 text-center leading-tight" style={{ fontFamily: "'Orbitron', ui-sans-serif, system-ui, -apple-system, 'Segoe UI', Roboto" }}>
            {/* Render per-character spans for stagger animation */}
        ...
        {/* <div className="absolute bottom-6 left-1/2 -translate-x-1/2 text-xs text-slate-400 opacity-70 select-none">
          Tip: Use <code className="font-mono">navigateWithPreload('/route')</code>
        </div> */}
