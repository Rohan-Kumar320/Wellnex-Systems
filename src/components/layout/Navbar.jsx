...
/**
 * Navbar with sliding underline for active nav links.
 * Replaces the desktop nav items rendering with a shared animated underline.
 *
 * - Uses `layoutId="nav-underline"` for the shared underline element.
 * - Respects prefers-reduced-motion.
 * - Uses CSS var `--accent-from` for underline color (fallback provided).
 */
...
export default function Navbar({ navigateWithPreload, debug = false }) {
  ...
  // read CSS vars (same safe approach you already used)
  const readCssVars = useCallback(() => {
    try {
      const s = getComputedStyle(document.documentElement);
      const navBg = s.getPropertyValue("--nav-bg").trim() || "rgba(5,5,5,0.95)";
      const navBgScrolled = s.getPropertyValue("--nav-bg-scrolled").trim() || navBg;
      const text = s.getPropertyValue("--text").trim() || "#0f172a";
      const muted = s.getPropertyValue("--muted").trim() || "#6b7280";
      const panel = s.getPropertyValue("--panel").trim() || "#ffffff";
  ...
  useEffect(() => {
    readCssVars();
    const root = document.documentElement;
    const mo = new MutationObserver(() => setTimeout(readCssVars, 12));
  ...
  async function safeNavigate(to) {
    ...
    if (typeof navigateWithPreload === "function") {
      try {
        ...
        if (debug) console.error("[Navbar] navigateWithPreload failed", err);
        routerNavigate(to);
      }
      return;
    }
    ...
    try {
      ...
      if (debug) console.error("[Navbar] routerNavigate failed", err);
      window.location.href = to;
    }
  }
  ...
  const desktopNavUnderlineStyles = {
    height: 3,
    borderRadius: 9999,
    background: `linear-gradient(90deg, ${cssVars.accentFrom}, ${cssVars.accentFrom})`,
    ...
    >
      <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-14 md:h-16">
          ...
          {/* Desktop nav: use a shared animated underline */}
          <div className="hidden md:flex items-center gap-8" style={{ color: cssVars.text, position: "relative" }}>
            {navItems.map((item) => {
              ...
              return (
                <div key={item.to} className="relative">
                  <button
                    type="button"
                    onClick={() => safeNavigate(item.to)}
                    className="transition-colors duration-200 text-sm px-1 py-2 relative"
                    aria-current={isActive ? "page" : undefined}
                    style={{
                      color: isActive ? cssVars.accentFrom : cssVars.text,
                  ...
                  {/* shared underline: only render for active item. using layoutId makes it animate between items */}
                  ...
                  {/* fallback non-animated underline for reduced-motion users */}
                  {prefersReduced && isActive && (
                    <span
                      style={{
                        position: "absolute",
      ...
      {menuOpen && (
        <motion.div
          ...
          className="md:hidden fixed left-0 right-0 top-[64px] px-4 py-6 z-40"
          role="menu"
          style={{ background: cssVars.navBg, color: cssVars.text }}
        >
          <ul className="space-y-4 text-center" style={{ color: cssVars.text }}>
            {navItems.map((item) => (
              <li key={item.to}>
